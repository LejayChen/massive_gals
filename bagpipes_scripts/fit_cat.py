...
redshift_sigmas = np.maximum(cat['Z_BEST']-cat['Z_BEST68_LOW'], (cat['Z_BEST68_HIGH']-cat['Z_BEST68_LOW'])/2).tolist()

# fitting
fit_cat = pipes.fit_catalogue(IDs, fit_instructions, load_gal, spectrum_exists=False,
                              cat_filt_list=filt_list_full, run=sys.argv[1], redshifts=redshifts, redshift_sigma=redshift_sigmas)
