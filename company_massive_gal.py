...
def bkg(mass_cen, ra_central, cat_all_z_slice_rand, coord_massive_gal_rand):
    ...
    while n < num_p:  # get several blank pointings to estimate background
        ...
        if sep2d.degree > 2.0/dis/np.pi*180:  # make sure the random pointing is away from any central galaxy (blank)
    ...
    return counts_gals_rand/float(num_p), total_mass_sat_rand, total_mass_sat_sf_rand
...
for z in np.arange(0.3, 2.0, 0.1):
    ...
    for gal in cat_massive_z_slice:
        ...
        count_gal_bkg, mass_sat_bkg, mass_sat_sf_bkg = bkg(mass_central, gal['RA'], np.copy(cat_all_z_slice), coord_massive_gal)
