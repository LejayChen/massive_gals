...
for gal in cat_massive_gal:
    ...
    mock_cat_zslice = mock_cat[abs(mock_cat['zKDEPeak']-eval(gal['z'])) < 2*0.044*(1+z)]
...
print(detected_gals)
print(all_gals)
print(detected_gals/all_gals)
print(detected_sfs/sf_gals)
print(detected_qs/q_gals)

# np.save('completeness_85.npy', detected_gals/all_gals)
# np.save('completeness_85_sf.npy', detected_sfs/sf_gals)
# np.save('completeness_85_q.npy', detected_qs/q_gals)
...
plt.ylim([0, 1])
plt.show()
# plt.savefig('../figures/spatial_completeness_85.png')
