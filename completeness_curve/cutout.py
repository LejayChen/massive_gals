...
def cutoutimg(filename, xc, yc, xw=25, yw=25, units='pixels', outfile=None,
    ...
    if newfile.shape[0] != 2*xw or newfile.shape[1] != 2*yw:
        warnings.warn('output figure ('+str(newfile.shape[0])+','+str(newfile.shape[1])
                      + ') has different dimension with user definition ('+str(2*xw)+','+str(2*yw)+') !', UserWarning)
    return newfile
