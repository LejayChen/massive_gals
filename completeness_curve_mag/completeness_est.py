...
if rank == n_procs-1:
    ...
    path = 'curves/'
    if as_func_of == 'mag':
        np.savetxt(path + 'comp_bootstrap_'+as_func_of+'.txt', all_curves)
    else:
        np.savetxt(path + 'comp_bootstrap_' + as_func_of + '_' + str(z_low)+'_'+str(z_high)+'.txt', all_curves)

# children processes (I said the calculation!)
else:
    for cat_name in cat_names:
        print('==============='+cat_name+'==================')
        mock_cat = Table.read(cat_stack_dir+'matched_cat_stack_'+cat_name+'_gal_cut_params.fits')
