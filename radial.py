...
cat_massive_gal = cat_gal[cat_gal['MASS_BEST'] > 10.9]
...
def bkg(ra_central, cat_all_z_slice_rand, coord_massive_gal_rand):
    ...
    while n < num_p:  # get several blank pointing's to estimate background
        ...
        if sep2d.degree > 2.5/dis/np.pi*180:  # make sure the random pointing is away from any central galaxy (blank)
            ...
            count_gal_rand, edges_rand = np.histogram(radius_neighbors_rand, bins=10**np.linspace(1, 2.75, num=100))
...
for z in np.arange(5, 5.1)/10.:
    ...
    for gal in cat_massive_z_slice:
        ...
        cat_neighbors_z_slice = cat_all_z_slice[abs(cat_all_z_slice['ZPHOT'] - gal['ZPHOT']) < 0.5]
        ...
        count_binned, bin_edges = np.histogram(radius_list, bins=10**np.linspace(1, 2.75, num=100))
