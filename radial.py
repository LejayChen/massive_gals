...
def bkg(ra_central, cat_neighbors_z_slice_rand, coord_massive_gal_rand, mode='count'):
    ...
    n = 0
    num_p = 2  # number of blank pointing's
...
for z in np.arange(7, 7.1)/10.:
    print('============='+str(z)+'================')
    cat_massive_z_slice = cat_massive_gal[abs(cat_massive_gal['ZPHOT'] - z) < 0.2]
    cat_all_z_slice = cat_gal[abs(cat_gal['ZPHOT'] - z) < 0.5]
    # cat_all_z_slice = cat_gal[cat_gal['SSFR_BEST']<-11]  #
    ...
    for gal in cat_massive_z_slice:
        ...
        cat_neighbors_z_slice = cat_all_z_slice[abs(cat_all_z_slice['ZPHOT'] - gal['ZPHOT']) < 4 * 0.03 * (1 + z)]
