...
def bkg(ra_central, cat_neighbors_z_slice_rand, coord_massive_gal_rand, mode='count'):
    ...
    while n < num_p:  # get several blank pointing's to estimate background
        ...
        while not same_field:
            id_rand = int(random()*len(cat_neighbors_z_slice_rand))
            ra_rand = cat_neighbors_z_slice_rand[id_rand]['RA'] + random()*2.0/dis/np.pi*180
            dec_rand = cat_neighbors_z_slice_rand[id_rand]['DEC'] + random()*2.0/dis/np.pi*180
...
for z in np.arange(6, 6.1)/10.:
    ...
    cat_all_z_slice = cat_all_z_slice[cat_all_z_slice['MASS_BEST'] > 10.1]  #
    ...
    for gal in cat_massive_z_slice:
        ...
        cat_neighbors_z_slice = cat_all_z_slice[abs(cat_all_z_slice['ZPHOT'] - gal['ZPHOT']) < 5 * 0.03 * (1 + z)]
        ...
        if mode == 'count':
            ...
            sat_counts_bkg = bkg(gal['RA'], cat_all_z_slice, coord_massive_gal, mode=mode)  # counts
    ...
    np.save(str(mode)+'_high', (radial_dist-radial_dist_bkg)/areas/float(massive_counts))
    np.save(str(mode)+'_high_err', np.sqrt(radial_dist+radial_dist_bkg)/areas/float(massive_counts))
    ...
    #     plt.savefig('radial_count_test_q.png')
    # plt.show()