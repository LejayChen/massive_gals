        ...
        if sep2d.degree > 1.4 / dis / np.pi * 180:  # make sure the random pointing is away from any central galaxy (blank)
            ...
            if isinstance(masscut_high, float):
                ...
                cat_neighbors_rand = cat_neighbors_rand[
                    cat_neighbors_rand[mass_keyname] < gal['MASS_MED'] + np.log10(eval(masscut_high))]

            if ssfq == 'all' and save_catalogs:
                cat_neighbors_rand.write('background_catalogs_45/' + cat_name + '_' + str(gal['NUMBER']) + '_' + str(
...
path = 'total_sample_evo_cen_masscut_newz/'
csfq = 'all'  # csf, cq, all
...
for z in z_bins:
    ...
    for gal in cat_massive_z_slice:
        ...
        #### TEMPERARY! ###  test larger isolation radius ###
        # cat_neighbors = cat_neighbors_z_slice[abs(cat_neighbors_z_slice['RA'] - gal['RA']) < 1.4 / dis / np.pi * 180]
        # cat_neighbors = cat_neighbors[abs(cat_neighbors['DEC'] - gal['DEC']) < 1.4 / dis / np.pi * 180]
        # # circular aperture cut, skip centrals that have no satellites
        # if len(cat_neighbors) == 0:
        #     print('No Satellite for', gal['NUMBER'])
        #     continue
        # else:
        #     ind = KDTree(np.array(cat_neighbors['RA', 'DEC']).tolist()).query_radius([(gal['RA'], gal['DEC'])],
        #                                                                              1.4 / dis / np.pi * 180)
        ...
        elif gal['MASS_MED'] < 11.23 - 0.16 * gal['zKDEPeak']:  # if set evolving masscut, apply additional mass cut
            isolated_counts -= 1
            continue
        else:
            isolated_cat.add_row(gal)
        ...
        if isinstance(masscut_low, float):  # fixed cut
            ...
            cat_neighbors = cat_neighbors[cat_neighbors[mass_keyname] > gal['MASS_MED']+np.log10(eval(masscut_low))]
        ...
        if isinstance(masscut_high, float):  # fixed cut
            ...
            cat_neighbors = cat_neighbors[cat_neighbors[mass_keyname] < gal['MASS_MED']+np.log10(eval(masscut_high))]
        ...
        if ssfq == 'all' and save_catalogs:
            cat_neighbors.write('satellite_catalogs_45_SXDS/' + cat_name + '_' + str(gal['NUMBER']) + '_' + str(
    ...
    if csfq == 'all' and ssfq == 'all':
        ...
        if masscut_low_host == 11.15 and save_catalogs:
            ...
            isolated_cat.write('massive_gal_positions_45/isolated_'+cat_name+'_'+str(masscut_low_host)+'_'+z_output+'.positions.fits', overwrite=True)
    ...
    spatial_weight, spatial_weight_err = spatial_comp(0.6, masscut_low, masscut_high, ssfq)
    if mode == 'count':
    ...
    # n_bkg_err = cal_error2(radial_dist_bkg, isolated_counts, spatial_weight, spatial_weight_err) / areas
