...
img = fits.open('chi2_8524_65.fits')
cutoutimg(img, gal['RA'], gal['DEC'], xw=width, yw=width, units='wcs', outfile='central.fits',
           overwrite=True, useMontage=False, coordsys='celestial', verbose=False, centerunits=None)
fig = aplpy.FITSFigure('central.fits', auto_refresh=True)
...
fig.show_circles([gal['RA']], [gal['DEC']],radius=0.04 / dis / np.pi * 180, color='b')
fig.show_circles(ra_list,dec_list,radius=0.03 / dis / np.pi * 180, color='g',linewidth=2.5)
...
fig.show_circles(ra_list_bkg,dec_list_bkg,radius=0.03 / dis / np.pi * 180,color='g',linewidth=2.5)
