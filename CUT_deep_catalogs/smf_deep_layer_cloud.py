...
print('=========rank='+rank+'======z='+str(round(z, 1))+'==========')
...
if cat_name == 'SXDS_uddd':
    cat_name = 'SXDS3_uddd'
    cat = Table.read('catalogs/s16a_' + cat_name + '_masterCat.fits')
else:
    cat = Table.read('catalogs/CUT3_' + cat_name + '.fits')
...
cat_random = Table.read('catalogs/s16a_' + cat_name + '_random.fits')
try:
...
if rank == 0:
    ...
    # save results
    np.save(filename + '_total', smf_dist_tot)
    np.save(filename + '_bkg', smf_dist_bkg_tot)
    np.save(filename + '_new', smf_dist_tot - smf_dist_bkg_tot)
    np.save(path + 'bin_edges', bin_edges)

    print('total number', round(sum(smf_dist)))
    print('total number in bkg', round(sum(smf_dist_bkg)))
    ...
    isolated_counts = len(cat_massive_z_slice_each)
